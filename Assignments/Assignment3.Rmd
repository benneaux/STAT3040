...
>>>>>>> 5d49dd1b1969b8bd20ed6d0bc691b2182dbbb9ae
...
**Answer:**
**Answer:** We are given (in the lecture notes) that the random observation vector $\pmb{X}$ - corresponding to the Regression Model defined in the question - has a **Multivariate Normal** distribution with **mean** vector $Z\beta$ and the **Covariance** matrix $\sigma_{\mathcal{W}}^{2}I$, where $I$ is an $n \times n$ identity matrix (Chapter 2: slide 14).

Therefore, the PDF of $\pmb{X}$ can be written as:

  $$
    f_{\pmb{X}}(\pmb{X};\pmb{\theta})=
    (2\pi)^{-\frac{n}{2}}|\sigma_{\mathcal{W}}^{2}I|^{-\frac{1}{2}}
    \exp\left\{
      -\frac{1}{2}({x}-{Z\beta})^T\left[{\sigma_{\mathcal{W}}^{2}I}\right]^{-1}({x}-{Z\beta})
    \right\}
  $$
  
Taking the log of this distribution gives is

  $$
    \mathcal{ln}(f_{\pmb{X}}(\pmb{X};\pmb{\theta})) = 
      -\frac{n}{2}ln(2\pi) 
      -ln(|\sigma_{\mathcal{W}}^{2}I|)-
      \frac{1}{2}({x}-{Z\beta})^T\left[{\sigma_{\mathcal{W}}^{2}I}\right]^{-1}({x}-{Z\beta})
  $$
  ...
  $$
    \begin{aligned}
      ...
      &=\frac{1}{2}E_{1}
        \left[
        -\frac{2}{2}ln(2\pi)-ln(|\sigma_{\mathcal{W}}^{2}I|)-\frac{1}{2}({x}-{Z\beta})^T\left[{\sigma_{\mathcal{W}}^{2}I}\right]^{-1}({x}-{Z\beta}) - \right.\\ 
        &\quad
        \left.\left(-\frac{2}{2}ln(2\pi)-ln(|\sigma_{\mathcal{W}}^{2}I|)-\frac{1}{2}({x}-{Z\beta})^T\left[{\sigma_{\mathcal{W}}^{2}I}\right]^{-1}({x}-{Z\beta})\right)
        \right] \\
      &=\frac{1}{2}E_{1}
        \left[
          ln\left(\frac{|\sigma_{\mathcal{W}}^{2}I|}{|\sigma_{\mathcal{W}}^{2}I|}\right)
        \right]
    \end{aligned}
  $$
