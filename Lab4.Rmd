...
2.	Generate the moving average series $v_{t}$ according to $v_{t}	= \frac{1}{2}u_{t}+ \frac{1}{3}u_{t-1} + \frac{1}{}u_{t-2}$ for $t = 3, 4, \cdots ,9$:	
```{r Q2}
v <- filter(u, sides = 1, c(1/2, 1/3, 1/6), method = ("convolution"))	
tidy(as.data.frame.ts(v))													
v2 <- filter(u, sides = 1, c(1/2,1/3,1/6))												
v2
```

3.	Generate the moving average series $w_{t}$	according to $w_{t}	= \frac{1}{2}u_{t+1} + \frac{1}{3}u_{t} + \frac{1}{}u_{t-1}$ for $t = 2, 3, \cdots ,8$:	
```{r Q3}
