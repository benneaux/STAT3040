...
4.	One strategy for dealing with this di  culty is called pre-whitening. The steps are:
...
6.	Generate the time series $Y_t$ = 10 + 0:6$X_t$ 2 + 1:2$X_t$ 4 + Vt where Vt is a Guassian White Noise series with mean zero and variance one:
```{r Q6}
...
7.	Fit an ARIMA model to $X_t$: acf(x)
```{r Q7}
pacf(x)
fit <- arima(x, c(1,0,0)) 
fit$coef
...
9.	Plot the cross-correlation function between the residuals of the tted model for the time series $X_t$ and the ltered model for $Y_t$:
```{r Q9}
pwx <- fit$residuals 
